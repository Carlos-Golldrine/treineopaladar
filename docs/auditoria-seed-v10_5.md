...
- A taxa relativa (~10,5% do banco) é **pior** que a anterior (37/1.227 = 3%), em números absolutos muito pior. Existe a aba `dedup_revisao` (466 linhas) e `auditoria_dedup`, mas o dedup **não foi aplicado até o fim** na aba principal — `dedup_revisao_flag` está 100% vazia na aba `vinhos`.
- IDs: 0 duplicados (bom — chaves primárias íntegras, mistura UUID + `v10-ml-*`).

### Encoding

- **0 células com mojibake** detectado (padrões `Ã©`, `â€™` etc.). Acentuação correta em pt-BR. ✔
...
3. **`pais` não normalizado:** `Australia` (100) vs `Austrália` (87); `Estados Unidos` (99) vs `EUA` (52); além de 1.225 como "Outros".
4. **`confianca_sensorial` com escala mista:** categórica (`alto/medio/baixo`) e numérica (70, 75, 65, 72…) na mesma coluna.
5. **Preços lixo:** 122 fora de (0, 5000] — alguns legítimos (Petrus R$94.895, DRC R$29.900, Lafite Vertical Edition R$265.050), mas há erros claros: espumante Miolo a R$0,00, "Carta Prime Free" a R$0,01, sucos a R$9,90, e um Monastrell comum a R$99.999 (placeholder evidente). 16 itens ≤ R$10.
6. **`teor_alcoolico`:** 50 valores fora de 4–25% — quase todos são os destilados infiltrados (gin 40%, cachaça 39%).
...
- **Ressalva crítica:** são URLs **externas de e-commerce** (hotlink). Podem quebrar a qualquer momento (mudança de CDN, parâmetro `v=` versionado nas VTEX) e há risco de bloqueio de hotlinking. Para produção é preciso **baixar e hospedar** (daí o `thumbnail_url` em 0%).
...
- **Brancos/espumantes com tanino ≥ 3: apenas 3 de 4.137 (0,07%)** ✔ — e 2 dos 3 são na verdade tintos Monte Paschoal classificados erroneamente como `espumante` (erro de tipo, não de tanino).
- Rosés com tanino ≥ 4: 0 de 660 ✔.
- Tintos com doçura ≥ 4: 181 — plausível (muitos "tinto suave" brasileiros no catálogo ML), mas vale amostragem.
...
- **Top países:** Brasil 3.433, Chile 2.195, Argentina 2.163, França 1.666, "Outros" 1.225, Portugal 1.133, Itália 1.120, Espanha 894 — distribuição ótima para o público brasileiro iniciante (35–54).
...
- **Safra:** 1957–2026, nenhuma fora de faixa ✔; `is_nv` marca 4.245 sem safra por natureza (espumantes etc.).

### Textos

- **`curiosidade_educacional`: 13.372 textos (91,6%), comprimento mediano 358 caracteres, 10.944 distintos** — textos genuínos por vinho, tom educativo correto p/ o público. Apenas 1 lixo encontrado ("aw2qa"). Guardrails embutidos: `superlativo_detectado` (38 marcados), `erro_factual` (1), `requer_revisao_factual` (38).
- **`harmonizacao_texto`: 6.515 (44,6%), mas só 54 textos distintos** — são templates por perfil de vinho (ex.: "Carnes vermelhas, queijos curados…" × 1.043). Funciona para gerar questões, mas não é harmonização específica por rótulo.
...
5. **Rastreabilidade:** `url_origem` (82,8%), `fonte_dado_principal`, `data_ultima_observacao`, flags de revisão editorial/factual — antes nada disso existia.
6. **Coerência sensorial excelente** (0,07% de violação tanino em brancos/espumantes).
7. **Encoding limpo** (0 mojibake).

### O que continua faltando / piorou

1. **`codigo_barras`: continua 0%** — scanner por código de barras segue inviável com este seed.
2. **`thumbnail_url`: continua 0%** — sem imagens hospedadas/otimizadas próprias; as 5.692 são hotlinks externos frágeis.
3. **Duplicatas pioraram em valor absoluto e relativo:** 37 → ~1.534 excedentes (10,5%). O dedup foi desenhado (abas auxiliares) mas não aplicado até o fim.
...
5. **Preços continuam sem validação completa:** 122 outliers/lixo (R$0,00, R$0,01, R$99.999), embora agora existam flags (`preco_invalido_origem`: 80 marcados) e a aba `vivino_resgate` com 80 casos aguardando decisão humana.
6. **Vazamento de não-vinhos:** 43 destilados/sucos/acessórios dentro de `vinhos` com `is_vinho=True`.
7. **Normalização pendente:** tipo (rose/rosé/rosado), país (Australia/Austrália, EUA/Estados Unidos), status_moderacao (auto_aprovado vs aprovado_automatico), confianca_sensorial (escala mista).
...
| Conteúdo educativo (cards/explicações) | **Alta** | curiosidade única em 91,6% |

**Recomendação obrigatória:** gerar questões só de uma *view* filtrada — `is_active=True` ∧ `is_vinho` real (excluir os 43 vazados por regex de nome) ∧ deduplicada (aplicar dedup por nome+produtor+safra; ~13k restantes) ∧ `status_moderacao != 'pendente'` se quiser rigor (sobram ~11,4k). Mesmo a view mais conservadora tem ~10x o banco anterior. Para questões com imagem, restringir aos 5.692 com URL (e idealmente após re-hospedar).

### Para o scanner futuro — **NÃO APTO ainda**

- **Por código de barras: inviável** (0% de `codigo_barras`, igual à versão anterior).
- **Por foto do rótulo (visual matching): parcialmente viável** — 5.692 imagens externas válidas dão base para começar um índice visual, mas (a) cobrem só 39% do catálogo, (b) excluem justamente a metade Mercado Livre/popular, e (c) são hotlinks que precisam ser baixados e hospedados antes de treinar/indexar qualquer coisa.
- Caminho recomendado: (1) baixar as 5.692 imagens agora, enquanto as URLs estão vivas; (2) preencher `codigo_barras` via API externa (Open Food Facts / GS1 / Cosmos) começando pelos top vendidos; (3) priorizar captura de imagem dos itens ML.
...
3. Baixar e hospedar as 5.692 imagens (urgente — URLs externas perecíveis) e popular `thumbnail_url`.
4. Normalizar `tipo`, `pais`, `status_moderacao`, `confianca_sensorial`.
5. Zerar os 122 preços inválidos (flags já existem; aba `vivino_resgate` aguarda decisão).
